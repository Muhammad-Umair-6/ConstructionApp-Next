...
function TabsList(props) {
  return (
    <TabsPrimitive.List
      data-slot="tabs-list"
      className={`bg-muted text-muted-foreground inline-flex h-9 w-fit items-center justify-center rounded-lg p-[3px] ${props.className || ""}`}
      {...props}
    />
  )
}
...
function TabsContent(props) {
  return (
    <TabsPrimitive.Content
      ...
      className={`flex-1 outline-none ${props.className || ""}`}
      {...props}
    />
  )
}
